...
func NewEmailNotifier(cfg *config.Config) *EmailNotifier {
	return &EmailNotifier{
		config: &cfg.SMTP,
		policy: cfg.Policy.Notification,
...
func (e *EmailNotifier) sendToRecipient(jobs []*models.Job, recipient string) error {
	...
	subject := e.renderTemplate(e.policy.EmailSubjectTemplate, map[string]string{
		"COUNT": strconv.Itoa(len(jobs)),
	})
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 ` + e.escapeHTML(e.policy.HeaderTitle) + `</h1>
            <p>` + e.escapeHTML(e.policy.HeaderSubtitle) + `</p>
        </div>
        
        <div class="summary">
            <strong>📊 Summary:</strong> ` + e.escapeHTML(e.renderTemplate(e.policy.SummaryTemplate, map[string]string{
		"COUNT": strconv.Itoa(len(jobs)),
	})) + `
        </div>
`)
	...
	for _, job := range jobs {
		...
		if job.JobDescription != "" {
			// Truncate description for compact email cards.
			description := job.JobDescription
			if len(description) > e.policy.JobDescriptionPreviewLimit {
				description = description[:e.policy.JobDescriptionPreviewLimit-3] + "..."
			}
			html.WriteString(fmt.Sprintf(`
            <div class="description">
                <strong>📝 %s</strong><br>%s
            </div>`, e.escapeHTML(e.policy.DescriptionTitle), e.escapeHTML(description)))
		}
		...
		if job.FinalReason != "" {
            ...
            <div class="reason">
                <strong>🎯 %s</strong> %s
            </div>`, e.escapeHTML(e.policy.ReasonTitle)+":", e.escapeHTML(job.FinalReason)))
		}

		if job.JobURL != "" {
			html.WriteString(fmt.Sprintf(`
            <a href="%s" class="apply-btn" target="_blank">🚀 %s</a>`, job.JobURL, e.escapeHTML(e.policy.ApplyButtonText)))
		}
	...
	html.WriteString(`
        <div class="personalized-message" style="background: linear-gradient(135deg, #ff9a9e 0%, #fecfef 50%, #fecfef 100%); padding: 20px; border-radius: 10px; margin: 30px 0; text-align: center; box-shadow: 0 4px 15px rgba(255, 154, 158, 0.3);">
            <div style="font-size: 18px; font-weight: bold; color: #d63384; margin-bottom: 10px; text-shadow: 1px 1px 2px rgba(0,0,0,0.1);">
                ` + e.escapeHTML(e.policy.PersonalizedTitle) + `
            </div>
            <div style="font-size: 16px; color: #8b5a96; font-style: italic; font-family: 'Georgia', serif;">
                ` + e.escapeHTML(e.policy.PersonalizedSubtitle) + `
            </div>
        </div>
        
        <div class="footer">
            <p>📧 ` + e.escapeHTML(e.policy.FooterLineOne) + `</p>
            <p>🕒 ` + e.escapeHTML(e.renderTemplate(e.policy.FooterLineTwoTemplate, map[string]string{
		"TIMESTAMP": time.Now().Format("January 2, 2006 at 3:04 PM"),
	})) + `</p>
            <p>💼 ` + e.escapeHTML(e.policy.FooterLineThree) + `</p>
        </div>
    </div>
</body>
</html>`)
...
func (e *EmailNotifier) renderTemplate(template string, values map[string]string) string {
	out := template
